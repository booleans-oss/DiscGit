  ...
  switch (event) {
    ...
    case "issue_comment": {
      ...
      const title = isCreated
        ? `[${payload.repository.full_name}] New comment on issue #${payload.issue.number}: ${payload.issue.title}`
        : `[${payload.repository.full_name}] Comment on pull request #${payload.issue.number} deleted`;
      MessageEmbed = {
        color: "F5DA81",
        author: {
          name: payload.sender.login,
    ...
    case "star": {
      MessageEmbed = {
        ...
        title: `[${payload.repository.full_name}] ${
          payload.action === "created" ? "New start added" : "Start removed"
        }`,
      };
      break;
    }
    ...
    case "release": {
      ...
      MessageEmbed = {
        ...
        }?v${payload.release.tag_name}] ${
          isReleased ? "New release" : "Release deleted"
        }`,
      };
      break;
    }
    ...
    case "pull_request_review_comment": {
      ...
      const isCreated = payload.action === "created";
      const title = isCreated
        ? `[${payload.repository.full_name}] New comment on pull request #${payload.pull_request.number}: ${payload.pull_request.title}`
        : `[${payload.repository.full_name}] Comment on pull request #${payload.pull_request.id} deleted`;
      MessageEmbed = {
        color: "F5DA81",
        author: {
          name: payload.sender.login,
      ...
      switch (payload.action) {
        ...
        case "closed": {
          MessageEmbed = {
            ...
            url: payload.pull_request.url,
            title: `[${payload.repository.full_name}] Pull request ${
