...
export default class WebhookHandler {
    ...
    async readWebhooks(): Promise<void> {
        ...
        for(let i = 0; i < Repos.length; i++) {
            ...
            const webhook: Webhook | undefined = await this.getWebhook(Repos[i].owner, Repos[i].repo, Repos[i].type);
    ...
    async createWebhook({owner, repo, channel, events, type}: HookData): Promise<void> {
        const webhook: Webhook = await this.app.APIHandler.createWebhook(owner, repo, events, type) as Webhook;
        this._data.push({owner, id: webhook.id, repo: repo, channel, type })
    }
    async updateWebhook({owner, repo, id, channel, events, type}: HookData): Promise<void> {
        const webhook = await this.app.APIHandler.updateWebhook(owner, repo, id, events, type) as Webhook;
        this._data.push({owner, id: webhook.id, repo: repo, channel, type});
    }

    async pingAll(): Promise<void> {
        for(let i = 0; i < this._data.length; i++ ) {
            await this.app.APIHandler.pingWebhook(this._data[i].id, this._data[i].repo, this._data[i].owner, this._data[i].type);
        }
    }
    ...
    isValidHook(hook: HookData): boolean {
        const MustHaveProperties: Array<"type" | "channel"|"repo" | "events" | "owner"> = ["type", "channel", "repo", "events", "owner"];
        let validity = true;
